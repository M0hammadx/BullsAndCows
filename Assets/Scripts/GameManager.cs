...
public class GameManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        #endregion
        */
        Destroy(LobbyManager.LM.gameObject);
    }
    ...
    void pastTurn(int id, string guessText)
    {
        ...
        newguess.GetComponent<DisplayGuessInfo>().guess = guess;
        newguess.transform.SetParent(Guess_list, false);
        ...
        {
            eliminatePlayer(id, Players[CurPlayer].pName);
            if (winState())
            {
    ...
    public void eliminatePlayer(int loser, string winner)
    {
        string lose = "", losenum = "";
        foreach (var player in Players)
            if (player.Id == loser)
        ...
        addNoti(winner + " x " + lose + " with " + losenum);
        foreach (var player in Players_List)
    ...
    {
        int cur = (CurPlayer + 1) % Players.Count();
        while (cur != CurPlayer && (!Players[cur].IN || Players[cur].AI))
            cur = (cur + 1) % Players.Count();
        if (cur == CurPlayer)
            return;
        MainTeam = Players[cur].teamNo;
    }
    ...
    public void addNoti(string content)
    {
        ...
        newNoti.transform.SetParent(noti_list.transform, false);
    }
